...
class SummaryDelegateCall extends DelegateDataFlowCall, TSummaryDelegateCall {
  ...
  override DataFlowCallable getARuntimeTarget(CallContext::CallContext cc) {
    exists(DispatchCall dc, DelegateArgumentToLibraryCallable arg |
      dc.getADynamicTarget().getSourceDeclaration() = c and
      dc.getArgument(pos) = arg and
      result = arg.getARuntimeTarget(_) and
      cc.(CallContext::NonDelegateCallArgumentCallContext).isArgument(dc.getCall(), pos)
    )
  }
